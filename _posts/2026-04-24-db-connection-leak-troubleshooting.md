...
트래픽이 아주 높지 않은데도 커넥션 풀이 자주 고갈되면 보통 "누수 아닌가?"부터 떠올리게 됩니다. 그런데 실무에서는 진짜 누수보다 **커넥션을 너무 오래 잡고 있는 경우**가 더 흔합니다.

둘을 구분하지 않으면 원인 분석이 계속 빗나가고, leak detection 로그만 보고 잘못된 결론을 내리기 쉽습니다.

## 먼저 구분할 것: 누수인가, 긴 점유인가

대략 아래처럼 나눠서 보면 빠릅니다.

- **진짜 누수**: 커넥션을 열고 반환하지 않아 풀로 돌아오지 않음
...
특정 시점에만 몰린다면 느린 작업일 가능성이 크고, 시간이 지나도 회복이 안 되면 누수 쪽을 더 의심할 수 있습니다.
